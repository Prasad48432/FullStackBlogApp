    ...
    <div className="bg-white shadow-lg rounded-lg p-5 pb-8 mb-8 outline outline-2 outline-black">
      ...
      {relatedPosts.map((post, index) => (
        <div key={index} className="flex items-center w-full mb-4">
            <img className="w-12 h-12 p-1 rounded-full ring-2 ring-gray-300 object-cover" src={post.featuredImage.url}/>
          <div className="flex-grow ml-4">
            <p className="text-gray-500 text-sm">{moment(post.createdAt).format('MMM DD, YYYY')}</p>
            <Link href={`/post/${post.slug}`} key={index}><h2 className='text-[15px]' >{post.title}</h2></Link>
          </div>
        </div>
      ))}
    </div>
