...
const Author = ({ author }) => (
  ...
  <div className="text-center mt-20 mb-8 p-12 relative shadow-lg rounded-lg bg-white  dark:bg-[#182a46] outline outline-2 outline-gray-300 dark:outline-[#182a46]">
    ...
    <h3 className="text-black dark:text-[#3b8e8a] mt-4 mb-1 text-xl font-bold">{author.name}</h3>
    <Link href={`/authorposts/${author.username}`}>
    <h3 className="text-black dark:text-[#3b8e8a] hover:text-gray-700 mt-4 mb-2 text-xl font-normal cursor-pointer">@{author.username}</h3>
    </Link>
